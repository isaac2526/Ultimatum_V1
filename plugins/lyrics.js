...
const text = m.body.slice(prefix.length + cmd.length).trim();
  ...
  if (validCommands.includes(cmd)) {
    if (!text) return m.reply(`Hello *_${m.pushName}_,*\n Here's Example Usage: _.lyrics Die With a Smile|Bruno Mars and Lady Gaga._`);

    try {
      await m.React('🕘');
      await m.reply('A moment, *𝐔𝐋𝐓𝐈𝐌𝐀𝐓𝐔𝐌-𝐕1* is generating your lyrics request...');

      if (!text.includes('|')) {
        return m.reply('Please provide the song name and artist name separated by a "|", for example: Die With a Smile|Bruno Mars and Lady Gaga..');
      }
      ...
      if (result && result.lyrics) {
        ...
        let buttons = [{
          ...
          },
          {
            name: "cta_url",
            buttonParamsJson: JSON.stringify({
              display_text: "sᴜᴘᴘᴏʀᴛ ɪsᴀᴀᴄ ᴀʀɪɴᴏʟᴀ ᴛᴇᴄʜ 🥺",
              url: `https://whatsapp.com/channel/0029Vb3Vx4jCBtx6yFD45S19`
            })
          },
          {
            name: "quick_reply",
            ...
            message: {
              ...
              interactiveMessage: proto.Message.InteractiveMessage.create({
                ...
                footer: proto.Message.InteractiveMessage.Footer.create({
                  text: "> *© ᴘᴏᴡᴇʀᴇᴅ ʙʏ ɪsᴀᴀᴄ ᴀʀɪɴᴏʟᴀ ᴛᴇᴄʜ™*"
                }),
